...
# File paths
out_file = "generated_queries_all.tsv"
out_file_sr = "generated_queries_all_sr.tsv"

translator = Translator()

# The query you want to resume from
resume_query = "what is the problem in the presence of uncertainty"
resume_flag = False  # Will turn True once we hit this query

with open(out_file_sr, "a", encoding="utf-8") as fOut:  # append mode
    for i, line in enumerate(open(out_file, encoding="utf-8")):
        try:
            query_en, abstract_en_trunc = line.strip().split('\t', maxsplit=1)
            ...
            if not row.empty:
                abstract_sr = row.iloc[0]["abstract_sr"]
                query_sr = translator.safe_translate(query_en, row_index=i)

                # write translated query and its sr abstract
                fOut.write(f"{query_sr.replace('\t', ' ').strip()}\t{abstract_sr.replace('\t', ' ').strip()}\n")
