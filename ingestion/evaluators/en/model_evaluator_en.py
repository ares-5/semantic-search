...
def compute_metrics(model, queries, passages, correct_idx, top_k=[1, 5, 10], batch_size=32):
    """
    Evaluate a retrieval model on a set of queries and passages.

    Args:
        model (SentenceTransformer): The embedding model used for encoding text.
        queries (List[str]): A list of query texts (e.g. titles, snippets).
        passages (List[str]): A list of passage texts (e.g. abstracts).
        correct_idx (List[int]): For each query, the index of its correct passage in `passages`.
                                 Example: if queries[i] matches passages[j], then correct_idx[i] = j.
        top_k (List[int], optional): Cutoff values for Recall@K. Default: [1, 5, 10].
        batch_size (int, optional): Batch size for encoding. Default: 32.

    Returns:
        Tuple[Dict[int, float], float]:
            - recalls: Dictionary mapping each K to Recall@K score.
                       Recall@K = fraction of queries where the correct passage
                       is found within the top K retrieved results.
            - mrr: Mean Reciprocal Rank. Average of 1/rank(correct_passage)
                   across all queries. Measures how high the correct result
                   is ranked on average.

    Notes:
        - High Recall@1 means the model often puts the correct passage first.
        - Recall@5/10 simulate a user scanning through top 5 or 10 results.
        - MRR captures ranking quality: closer to 1.0 if correct results are consistently near the top.
    """
